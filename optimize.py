...
class Recommender:
    ...
    def _get_for_component(self, component: DetectedComponent, detected: dict[str, DetectedComponent]) -> list[Recommendation]:
        ...
        category_map: dict[str, str] = {
            "framework": "frameworks", "language": "languages", "testing": "testing",
            "database": "databases", "css": "css", "ci_cd": "ci_cd", "mobile": "mobile", "infra": "infra",
        }
        ...
        for parent_name in entry.get("extends", []):
            parent_component = DetectedComponent(
                category=component.category, name=parent_name,
        ...
        for skill_name in entry.get("skills", []):
            if skill_name not in {r.name for r in recs if r.kind == "skill"}:
                recs.append(Recommendation(
                    category=component.category, name=skill_name, kind="skill",
        ...
        for mcp_name in entry.get("mcps", []):
            if mcp_name not in {r.name for r in recs if r.kind == "mcp"}:
                recs.append(Recommendation(
                    category=component.category, name=mcp_name, kind="mcp",
        ...
        for hook_event, hook_commands in entry.get("hooks", {}).items():
            for cmd in hook_commands:
                recs.append(Recommendation(
                    category=component.category, name=f"hook_{hook_event}_{len(cmd)}", kind="hook",
                    summary=f"{hook_event}: {cmd}",
                    reason=f"{hook_event} hook for {component.name}",
        ...
        for rule_text in entry.get("rules", []):
            ...
            recs.append(Recommendation(
                category=component.category, name=rule_name, kind="rule",
                summary=f"Rule: {rule_text}",
    ...
    def _add_implied_recommendations(self, results: list[Recommendation], scan: ScanResult, seen: set[str]):
        ...
        if has_typescript and not has_typecheck_hook:
            results.append(Recommendation(
                ...
                summary="PostToolUse: npx tsc --noEmit",
    ...
    def _skill_install_steps(self, skill_name: str) -> list[str]:
        return [
            f"Check if '{skill_name}' is already in ~/.claude/skills/",
            f"Symlink into ~/.claude/skills/ if not already present",
        ]
    ...
    def _mcp_install_steps(self, mcp_name: str) -> list[str]:
        ...
        if config:
            return [f"Add to mcp.json: {json.dumps({mcp_name: config}, indent=2)}"]
...
class Installer:
    ...
    def install(self, recommendations: list[Recommendation], dry_run: bool = False) -> list[str]:
        ...
        for rec in skills_to_add:
            skill_src = self.skills_dir / rec.name
            if skill_src.exists():
                actions.append(f"  Skill '{rec.name}' already installed")
            else:
                actions.append(f"  Skill '{rec.name}' not found in skills directory")
        ...
        for rec in hooks_to_add:
            ...
            if match:
                ...
                if hook_entry not in settings["hooks"][event]:
        ...
        for rec in rules_to_add:
            ...
            rule_file = rules_dir / f"{rec.category}.md"
...
def cmd_scan(args: list[str]) -> None:
    ...
    output["components"] = [
        {"category": c.category, "name": c.name, "confidence": c.confidence, "detail": c.detail, "files": c.files}
...
def cmd_recommend(args: list[str]) -> None:
    ...
    for c in sorted(scan.components, key=lambda x: (-x.confidence, x.category)):
        print(f"  {c.category}: {c.name} ({(c.confidence*100):.0f}% confidence)")

    if recommendations:
        kinds: dict[str, list[Recommendation]] = {}
        for r in recommendations:
            kinds.setdefault(r.kind, []).append(r)

        print(f"\nRecommended tools ({len(recommendations)} total):")
        for kind, items in sorted(kinds.items()):
            label = kind.capitalize() if kind != "mcp" else "MCP servers"
            if kind == "hook":
                label = "Hooks"
            elif kind == "skill":
                label = "Skills"
        ...
        print(f"\nTo install, run: python optimize.py install <recommendations.json>")
        print(f"Or let Claude walk through it with /optimize\n")
    else:
        print(f"\nNo specific recommendations for this stack.")
        print("You can extend the knowledge base in recommendations.json\n")
...
def cmd_report(args: list[str]) -> None:
    ...
    if mcp_config.exists():
        try:
            ...
            print(f"\nMCP servers configured ({len(mcps)}):")
            for name in mcps:
                print(f"  - {name}")
        except (json.JSONDecodeError, OSError):
            print(f"\nCould not parse mcp.json")
    else:
        print(f"\nNo mcp.json found")
                    ...
                    for event, entries in hooks.items():
                        for entry in entries:
                            if isinstance(entry, dict) and "hooks" in entry:
                                ...
                                total += 1
                    print(f"\nHooks configured ({total} total):")
                    for event, entries in hooks.items():
                        for entry in entries:
                            if isinstance(entry, dict) and "hooks" in entry:
                                for h in entry["hooks"]:
                                    cmd = h.get("command", str(h)[:60])
                                    suffix = f" {args[0][:40]}..." if (args := h.get("args", [])) and isinstance(args, list) and args and isinstance(args[0], str) else ""
